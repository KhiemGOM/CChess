    ...
    if (is_within_field(target) && target != pos) {
        ...
        if (find_result.has_value()) {
            ...
            if (((type == e_king && piece_at_target->type == e_rook) ||
                ...
                if (pos.y == 0 || piece_at_target->pos.y == 0) {
                    switch (type) {
                        case e_king:
                            pos.x = 6;
                ...
                if (pos.y == 7 || piece_at_target->pos.y == 7) {
                    switch (type) {
                        case e_king:
                            pos.x = 2;
    ...
    if (is_within_field(target) && target != pos) {
        ...
        if (find_result.has_value()) {
            ...
            if (((type == e_king && piece_at_target->type == e_rook) ||
                ...
                move_cleanup();
                piece_at_target->move_cleanup();
                if (is_being_checked(game_board, target, color))
                    return move_state::invalid_move;
                if (is_check(game_board, invert(color))||piece_at_target->is_check(game_board, invert(color))) //WRONG: the rook and king should all be checked
                    return move_state::check;
                return move_state::valid_capture;
            }
            if (is_valid_capture(game_board, target) && piece_at_target->color != color) {
 ...
 **/
bool pieces::is_being_checked(board &game_board, position target, color_enum _color) const {
